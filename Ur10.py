...
# De la tabla
T01 = T.subs({d: 0.128, a: 0, alpha: sp.pi/2, theta: theta1})
sp.pprint(T01)
T12 = T.subs({d: 0.128, a: 0.612, alpha: sp.pi, theta: theta2})
sp.pprint(T12)
T23 = T.subs({d: 0.128, a: 0.572, alpha: -sp.pi, theta: theta3})
sp.pprint(T23)
T34 = T.subs({d: 0.128, a: 0, alpha: -sp.pi/2, theta: theta4})
sp.pprint(T34)
T45 = T.subs({d: 0.116, a: 0, alpha: sp.pi/2, theta: theta5})
sp.pprint(T45)
T56 = T.subs({d: 0.092, a: 0, alpha: 0, theta: theta6})
sp.pprint(T56)
...
joint1,joint2,joint3,joint4,joint5,joint6 = 0,0,0,0,0,0
# Definimos el modelo DH para el ABB-IRB-120-3-0-6
IRB_robot = rtb.DHRobot(
    [
        rtb.RevoluteDH(d=0.128, a=0, alpha=np.pi/2, qlim=[-2*np.pi, 2*np.pi]),
        rtb.RevoluteDH(d=0.176, a=0.612, alpha=np.pi, offset=np.pi/2, qlim=[-2*np.pi, 2*np.pi]),
        rtb.RevoluteDH(d=0.140, a=0.572, alpha=-np.pi, qlim=[-0.8611*np.pi, 0.8611*np.pi]),
        rtb.RevoluteDH(d=0.128, a=0, alpha=-np.pi/2, offset=-np.pi/2, qlim=[-2*np.pi, 2*np.pi]),
        rtb.RevoluteDH(d=0.116, a=0, alpha=np.pi/2, qlim=[-2*np.pi, 2*np.pi]),
        rtb.RevoluteDH(d=0.092, a=0, alpha=0, qlim=[-2*np.pi, 2*np.pi])
...
Ttotal=IRB_robot.fkine([0,0,-np.pi/2, -np.pi/2, -np.pi/2, 0 ])
print(Ttotal)
...
IRB_robot.qz=[0, 0, 0, 0, 0, 0]

IRB_robot.qr=[0,0,-np.pi/2, -np.pi/2, -np.pi/2, 0 ]
# Graficar con controlador
q1 = np.array([[0, 0, 0, 0, 0, 0]])
IRB_robot.teach(IRB_robot.qr)